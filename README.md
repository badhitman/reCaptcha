...
> Внимание! Данный привязчик хорошо показал себя в классическом **ASP.Net Core 2.2**, но не в React web-API ASP.Net Core 3.1. "Причина", предположительно, в применяемой методике отпарвки данных клиентом в теле запроса `application/json` и получением этих данных в контроллере с маркером `Post([FromBody]...`. При использовании в данном типе проектов я на стороне клиента "исправляю" имя поля "с девисами" на "подчёркивание".
