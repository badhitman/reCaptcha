...
Если вы хотите прочитать из кеша напрямую вым нужно имя хранимых данных.
Что бы получить имя хранимой оценки нам нужно получить стабильный идентификатор сессии.
В данном случае этот идентификатор хранится в дополнительном параметре сессии **"ClientId"** вот таким способом:
```c#
...
Фильтр что ыб заработал нужн оподключить как Scoped сервис:
```c#
