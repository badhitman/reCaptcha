...
Версия **v3** только оценивает пользователя по шкале от **0.0** до **1.0**, где:
- `0.0` обозначаются например боты или спамеры
- `1.0` оценка для "хорошего" пользователя

Оценка между **0.0** и **1.0** (например: **0.3** или **0.9**) означает, что **reCaptcha** не уверен с кем имеет дело и судя по значению можно понять к чему **reCaptcha** больше склоняется.
