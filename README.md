# reCaptcha
reCaptcha // c# asp.net standart 2.0

Решение для внедрения на свой сайт **reCaptcha**

Версий **reCaptcha** существует (на момент написания) две (**v2** и **v3**), но **v2** в свою очередь делится на три под-версии.
- [v3](https://developers.google.com/recaptcha/docs/v3) - на самом деле ни какая не капча. Эта версия в своей работе не выводит капчи, а производит оценку посетителя вашего сайта. Эта версия функционирует без какого либо взаимодействия с самим пользователем
- v2 - привычная/классическая капча. Задачки с выбором картинок и прочие проверочные "тесты зрения и нервов" пользователя. Эта версия разделена на три подверсии:
...
## v3
Оценка основана на взаимодействиях пользователя с вашим сайтом и позволяет вам предпринимать соответствующие решения для реагирования на посетителей с учётом этой оценки сервисом **reCaptcha**.
Таким образом можно вводить или пропускать те или иные уровни защиты или дополнительные проверки на вашем сайте.
Эта версия не подразумевает ни какой отображаемой капчи в принципе. Собственно, называть эту версиею капчей не уместно т.е. на деле это НЕ капча, а инструмент оценки посетителя сайта с целю определения вероятности того что это "хороший" или "плохой" посетитель.
Версия **v3** оценивает посетителя по шкале от **0.0** до **1.0**, где:
- `0.0` обозначаются например боты или спамеры
- `1.0` оценка для "хорошего" пользователя

Оценка может оказаться в диапазоне между крайними значениями **0.0** и **1.0** (например: **0.3**, **0.4** или **0.9**) означает, что **reCaptcha** не уверен с кем имеет дело и судя по значению можно понять к чему **reCaptcha** больше склоняется.
...
> Внимание! Данный привязчик хорошо показал себя в классическом **ASP.Net Core 2.2**, но не в **React web-API ASP.Net Core 3.1**. "Причина", предположительно, в применяемой методике отпарвки данных клиентом в теле запроса `application/json` и получением этих данных в контроллере с маркером `Post([FromBody]...`. При использовании в проектах подобного типа, нужно стороне клиента "исправлять" имя поля "с дефисами" на "подчёркивание".
...
Серверная часть от **v2 Checkbox widget** ни чем не отличается. Различие только в том как ведёт себя визуальная часть
